...
public :
    ...
    int nb_out;
    bool is_repeated;
    bool prefix_is_repeated;// is the k-1 suffix of this kmer is repeated in the reference genome
    } info_type;
...
public :
     ...
     */
    void writeBreakpoint(int bkt_id, string& chrom_name, uint64_t position, string& kmer_begin, string& kmer_end, int repeat_size, string type, string found_snp, bool repeat_in_genome_kmer_begin =false, bool repeat_in_genome_kmer_end = false);
     ...
     */
    int recent_hetero();
    /** help to find hetero sites with snp around (<k-1 mer)
    */
    int recent_snp();
    /**
     */
    ...
    /** Set value of recent_snp
     */
    void recent_snp(int value);
private :
...
void FindBreakpoints<span>::notify(Node node, bool is_valid)
{
    ...
    if(in_graph && is_valid)
    {
        ...
        if (this->recent_snp()>0)
        {
            m_recent_snp--;
            if (this->recent_hetero()>0)
            {
                m_recent_snp=0;
            }
        }
...
template<size_t span>
void FindBreakpoints<span>::writeBreakpoint(int bkt_id, string& chrom_name, uint64_t position, string& kmer_begin, string& kmer_end, int repeat_size, string type, string found_snp, bool repeat_in_genome_kmer_begin, bool repeat_in_genome_kmer_end  ){
    fprintf(this->finder->_breakpoint_file,">bkpt%i_%s_pos_%lli_fuzzy_%i_%s_%s %s left_kmer\n%s\n>bkpt%i_%s_pos_%lli_fuzzy_%i_%s_%s %s right_kmer\n%s\n",
        bkt_id,
        chrom_name.c_str(),
...
template<size_t span>
int FindBreakpoints<span>::recent_snp()
{
    return this->m_recent_snp;
}
template<size_t span>
...
template<size_t span>
void FindBreakpoints<span>::recent_snp(int value)
{
    this->m_recent_snp = value;
}
...
void FindBreakpoints<span>::store_kmer_info(Node node)
{
    ...
    this->m_current_info.prefix_is_repeated=this->m_ref_bloom->contains(min(prefix,prefix_rev));
//	if(this->finder->_hete_insert) //alwayss fill repeat info
